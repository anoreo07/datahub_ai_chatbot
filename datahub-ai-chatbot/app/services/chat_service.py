...
from app.services.chat.flows.domain_glossary import (
    GLOSSARY_CONCEPT_KEYWORDS as _GLOSSARY_CONCEPT_KEYWORDS,
    TERM_DOMAIN_CACHE as _TERM_DOMAIN_CACHE,
    domain_scoped_term_answer as _domain_scoped_term_answer,
    glossary_concept_members as _glossary_concept_members,
    term_domain_map as _term_domain_map,
    term_linked_datasets as _term_linked_datasets,
)
from app.services.chat.pipeline import (
    FALLBACK_INTENT_MAP,
    STANDARD_TAXONOMY,
    background_ragas_eval,
    build_grounded_fallback as _build_grounded_fallback,
    field_meaning as _field_meaning,
    generate_or_fallback,
    log_interaction_async,
    postprocess_response,
    qu_primary_intent as _qu_primary_intent,
    unify_intent_label as _unify_intent_label,
)
...
class ChatService:
    ...
    async def _log_interaction_async(self, op: str, **kwargs: Any) -> None:
        ...
        await log_interaction_async(op, **kwargs)
    ...
    async def _postprocess_response(
        ...
        return await postprocess_response(self._ctx, res, t_start, uid, cid, question)
    ...
    async def _background_ragas_eval(
        ...
        await background_ragas_eval(self._ctx, trace_id, question, answer, contexts, history)
    ...
    async def _generate_or_fallback(
        ...
        return await generate_or_fallback(
            generator, question, results, intent, history, on_token, recommendation
        )
    ...
    async def _try_metadata_listing(
        ...
        return await try_metadata_listing(
            self._ctx, question, user_ctx, trace_id, cid, self._postprocess_response, _t_start
        )
    ...
    async def _answer_direct_field_op(
        ...
        """Answer a self-contained field question directly from schema metadata."""
        from app.services.chat.flows.direct_field import answer_direct_field_op
        return await answer_direct_field_op(
            self._ctx, query, uid, cid, trace_id=trace_id, understanding=understanding
        )
    ...
    async def _comparison_flow(
        ...
        from app.services.chat.flows.comparison import comparison_flow
        return await comparison_flow(
            self._ctx,
            query,
            entity_names,
            user_ctx,
    ...
    def _deterministic_comparison(
        ...
        from app.services.chat.flows.comparison import deterministic_comparison
        return deterministic_comparison(entity_details, aspects)
